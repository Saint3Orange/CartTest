...
@section('content')
    ...
    <table id="cart" class="table table-hover table-condensed">
        ...
        @if(session('cart'))
            @foreach(session('cart') as $id => $details)
                ...
                <tr>
                    ...
                    <td data-th="Price">{{ $details['price'] }}р</td>
                    <td data-th="Quantity">
                        <input type="number" value="{{ $details['quantity'] }}" class="form-control quantity" />
                    </td>
                    <td data-th="Subtotal" class="text-center">{{ $details['price'] * $details['quantity'] }}р</td>
        ...
        <tfoot>
        <tr class="visible-xs">
            <td class="text-center"><strong>Итого {{ $total }} р</strong></td>
        </tr>
        <tr>
            <td><a href="{{ url('/') }}" class="btn btn-warning"><i class="fa fa-angle-left"></i> Продолжить покупки</a></td>
            <td colspan="2" class="hidden-xs"></td>
            <td class="hidden-xs text-center"><strong>Итого р{{ $total }}</strong></td>
        </tr>
        </tfoot>
    </table>
